...
outputnumber=537
ncores=320
path=r"C:\Users\nakaj\OneDrive\Desktop\2025Work\HalfMoonCollision\\"
outputpath=path
axesdim=20
cutoff='Yes'
...
def melting(cutoff):    #Determines which particles are melted
    ...
    with open(f'{path}/SortedData.txt','r') as file:
        ...
        #within that distance, if cutoff is enabled.
            ...
            particle = namedtuple('particle', ['id','tag','x','y','z','r','press','temp'])
            particles=[]
            rlist=[]
            ...
            for line in file:
                ...
                particles.append(particle(id,tag,xx,yy,zz,r,press,temp))
                rlist.append(np.linalg.norm(r))

            plt.figure(figsize=(8,5))
            plt.hist(x=rlist,bins=500,range=(0,max(rlist)),color='blue',alpha=0.4)
            plt.xlabel(f'Radius ({r0:.1e} m)')
            plt.ylabel('Count')
            plt.title('Particle Distribution')
            plt.show()

            cutoff = float(input("    Enter the cutoff for melting analysis (1e6 m): "))
